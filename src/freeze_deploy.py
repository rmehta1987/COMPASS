...
import argparse, ast, hashlib, json, shutil, subprocess, sys, time
...
# The commit whose bundle the tracked serving-machine report certifies. The
# manifest compares the current bundle against it file by file so that a report
# proving an older bundle is never presented as proving this one.
PROVEN_ON_SERVING_MACHINE_COMMIT = "6416094"
...
def proof_chain(root: Path, files: dict) -> dict:
    """Which hashed files differ from the bundle the serving machine proved, and
    whether each difference is code-neutral (AST identical once docstrings are
    dropped) or a code change. Falls back gracefully outside a git checkout."""
    c = PROVEN_ON_SERVING_MACHINE_COMMIT
    try:
        then = json.loads(subprocess.run(["git", "show", f"{c}:deploy/manifest.json"],
                                         capture_output=True, text=True, check=True).stdout)["files"]
    except Exception as e:                                   # noqa: BLE001
        return {"serving_machine_proved_commit": c, "comparison": f"unavailable: {e}"}
    diffs = {}
    for f in sorted(set(then) | set(files)):
        o, n = then.get(f, {}).get("sha256"), files.get(f, {}).get("sha256")
        if o == n:
            continue
        d = {"sha256_then": o, "sha256_now": n}
        if f.endswith(".py") and o and n:
            osrc = subprocess.run(["git", "show", f"{c}:deploy/{f}"], capture_output=True, text=True).stdout
            nsrc = (root / f).read_text()
            d["ast_identical_modulo_docstrings"] = _strip_docstrings(osrc) == _strip_docstrings(nsrc)
        diffs[f] = d
    return {
        "serving_machine_proved_commit": c,
        "serving_machine_report": "out/smoke_report_x86_64_Wright.json",
        "files_differing_from_proved_bundle": diffs,
        "identical_to_proved_bundle": not diffs,
        "what_this_means": (
            "The tracked serving-machine report certifies the bundle at the commit named "
            "above. Files listed as differing have changed since; a file with "
            "ast_identical_modulo_docstrings=true changed only in docstrings or comments, "
            "false means code (including message strings) changed. The current bundle is "
            "proven wherever deploy/smoke_test.py has been run against THIS manifest: on the "
            "training machine at every freeze (report out/smoke_report_aarch64_spark-2500.json, "
            "not tracked). Re-running the smoke test on the serving machine and committing its "
            "report re-closes the chain." if diffs else
            "The tracked serving-machine report certifies exactly this bundle."),
    }
...
def main() -> int:
    ...
    manifest = {
        ...
        "files": files,
        "proof_chain": proof_chain(root, files),
    }
