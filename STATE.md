...
build: 3dc8415eccfe
last green: 4059776   (VERIFIED 2026-09-04 by ./check.sh, GREEN)
...
- [ ] B2 schema per the brief (replaces the 16a draft's models): InventoryVariable(role, key|None, in_instrument, absent_reason, resolution verified|found_by_search, confident), PaperInventory(pmid, exposures, outcomes, covariates, design, direction, notes); B2a found_by_search / confident=False rows excluded by the harness and counted; B2b covariate-boundary statement in the key's README; authoring guide benchmark/PAPER_INVENTORY_GUIDE.md
- [ ] B3 modal covariate set from the inventory; recall/precision per paper and pooled, the same for the modal set, and the margin; B3a degeneracy check (design distribution, direction base rate) as a stated prerequisite
