...
build: 3dc8415eccfe
last green: 74170b9   (VERIFIED 2026-09-05 by ./check.sh, GREEN)
...
- [x] 15 baseline score, once, on a tag — DONE 2026-09-04 (BASELINE.md at c22185e: 0/28, denominator 48, verdicts ok) — split 2026-09-04 after items 1-14, one commit each:
...
## LOOP COMPLETE 2026-09-04
Items 1-15 done. Baseline: artefacts/b3-20260904/BASELINE.md (matched 0 / scored 28 / denominator 48, all verdicts ok), tag baseline-3dc8415eccfe-b3-20260904-stamped. Open for the operator: (1) the baseline's frame (D1 narrow) shares no subject with the bibliography, and the key records an outcome for only 3 of 16 papers, so 0 is expected under this design and says little about hypothesis quality; a frame drawn to cover the papers' strata, or a widened key, would make the after measurable; (2) 7 backend_error pairs at the tail of the run (cause unknown, empty stderr); (3) 11 of 41 records discarded by validator:temporality for mediators under a cross-sectional design; (4) DEFERRED D3 effort pin + A/B; (5) the stray unsuffixed tag may be deleted by the operator; (6) compass-score holds the key files untracked under .git/info/exclude — never merge that clone's working tree anywhere.

## Brief 2 (2026-09-04) · specification benchmark — items, one commit each
- [x] B0 ceiling field in baseline_score.py + first line of BASELINE.md (re-render in compass-score, commit the amendment; the tag and the 0/28 stand)
- [ ] B1 temporality discards (11 of 39 records, 18 mediator + 2 descendant critiques): prompt fix in agent/specifier.py telling the model the cross-sectional constraint BEFORE it assigns roles; validator unchanged; accept = seeded re-run of the same 48 pairs (seed 0) with a materially lower discard rate; report both rates
- [x] B2 schema per the brief (replaces the 16a draft's models): InventoryVariable(role, key|None, in_instrument, absent_reason, resolution verified|found_by_search, confident), PaperInventory(pmid, exposures, outcomes, covariates, design, direction, notes); B2a found_by_search / confident=False rows excluded by the harness and counted; B2b covariate-boundary statement in the key's README; authoring guide benchmark/PAPER_INVENTORY_GUIDE.md
- [x] B3 (code in paper_inventory.py: modal_covariates, degeneracy; rendered by B5) modal covariate set from the inventory; recall/precision per paper and pooled, the same for the modal set, and the margin; B3a degeneracy check (design distribution, direction base rate) as a stated prerequisite
- [x] B4 posed-pair driver (pipeline/pose.py from 16a) + a synthetic inventory of 3-4 papers with real dictionary keys used by the tests; the run's artefacts state the inventory was synthetic
- [x] B5 benchmark/specification_score.py, harness mode 2: same refusals/verdicts/loading; covariate recall/precision + modal baseline + margin, design agreement with base rate (or omitted per B3a), direction agreement with base rate, excluded rows, in_instrument=False papers counted apart, discovery rate beside never pooled, verdicts, ceiling, n on every figure
- [~] B1 accept half: b4-20260904 (same 48 pairs, seed 0, new prompt) ran 22:50-01:04 and lost 32 pairs to a backend outage from the 16th pair on (every call exit 1, empty stderr; CLI healthy again at 01:10); on its 14 records: 1 temporality discard (7%) vs 3 on the same 14 in b3 · b5-20260905 launched 2026-09-05 ~01:20 with --skip-recorded artefacts/b4-20260904 --retry-pause 300 for the 32 unsettled pairs (~4.5 h); the after-rate = temporality discards over records in b4 ∪ b5, reported beside b3's 11/39; both runs' artefacts stay UNTRACKED (accept measurement, not a baseline)
- STOPPED 2026-09-04 23:15 after B0-B5 green: the inventory is the operator's, on scoring-key; nothing downstream of it runs until it lands. Runbook: benchmark/PAPER_INVENTORY_GUIDE.md; posed run = `python -m pipeline.pose <run_id> --pairs run/posed.jsonl --inventory 'scoring-key <sha>' --allow-unestimable --k 5 --workers 5`, then commit/push/stamp/tag as for b3; scoring in compass-score = `python -m benchmark.specification_score --sha <tree_sha> --discovery artefacts/b3-20260904/BASELINE.json <emitted posed artefacts>` (BASELINE.json is written by baseline_score from now on; re-run it once there to produce it)
(the 16a draft below is superseded by B2/B4; kept for the audit trail)
## 16 · specification benchmark (after the loop; operator request 2026-09-04)
- [x] 16a schema benchmark/paper_inventory.py + posed-pair driver pipeline/pose.py (tests in tests/test_paper_inventory.py, tests/test_pose.py)
- [ ] 16b the inventory itself: OPERATOR, on scoring-key only, as benchmark/paper_inventory_key.py::INVENTORY — start from `python -c "from benchmark.paper_inventory import template; print(template())"`, one PaperInventory per paper, every key checked with env.tools.resolve_variable (validate_against_dictionary), labels are paper content and never leave that branch
- [ ] 16c posed run, in a generation clone: `python -c "from benchmark.paper_inventory import load_inventory, posed_pairs; import json; [print(json.dumps(r)) for r in posed_pairs(load_inventory())]" > run/posed.jsonl` is run on the SCORING side (it needs the key), then only run/posed.jsonl (keys, no labels) travels to the generation clone; there: `python -m pipeline.pose <run_id> --pairs run/posed.jsonl --dry-run`, then without --dry-run with --allow-unestimable --k 5 --workers 5; commit + push artefacts/<run_id>/, stamp (pipeline.generation_env.stamp(require_pushed=True) + pipeline.run.stamp_run), commit + push, tag posed-<hash>-<run_id>
- [ ] 16d scoring mode for the harness (not yet written): per paper, covariate recall/precision of the hypothesis's adjustment set against the inventory's in-instrument covariate keys (by role and overall), design agreement, direction agreement, plus the discovery rate from the funnel baseline beside it; same refusals, verdicts and stamp discipline as benchmark/baseline_score.py; posed records never enter the funnel denominator (screened_from=0 is asserted on every record)
