...
`targets.json` (1,352, byte-reproducible from the build; `src/compass_build.py`
later emits 1,353, the extra being `m2:Q785~2`, see `RESULTS.md` §1). Selector
...
4. **Fine-tuning is weaker than before, on two independent grounds.** Scaling the
   ...
   > **Superseded on this point, 2026-09-03.** Fine-tuning was then measured
   > (`RESULTS.md` §4, `CHARACTERISATION.md`): contrastive fine-tuning of
   > `bge-small` on 13,528 synthetic pairs lifted R@1 from 0.375 to **0.567** on
   > the same 224 rows, with the in-batch-negatives-only configuration winning
   > and mined hard negatives losing, which is consistent with §2d. The
   > gold-wording-leakage share of that gain was measured and does not survive
   > (`FUSION.md` §1). The scaling argument in §2a stands.
