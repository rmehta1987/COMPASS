...
on the row. Nothing here is re-typed from a prior report. Most `out/` artifacts, the
fixtures and the checkpoint are on the training machine, not in this public tree;
[`PROVENANCE.md`](PROVENANCE.md) lists every artifact this document cites with its sha256,
and for the headline figures the command that regenerates them and the tracked file that
reproduces them. A backticked path below may be tracked or training-machine-only.
...
| acceptance test | [`deploy/smoke_test.py`](deploy/smoke_test.py) — exits 0 only if arms S, F and I reproduce to the digit; passes on the Spark at every freeze, and passed on the x86 serving machine for the bundle at commit 6416094 ([`out/smoke_report_x86_64_Wright.json`](out/smoke_report_x86_64_Wright.json)); the code files have since changed in docstrings and hint strings only (`deploy/manifest.json::proof_chain`) |
