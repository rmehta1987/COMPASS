...
(https://github.com/rmehta1987/COMPASS). The rule holds on this branch; it does **not**
yet hold on `origin/main`, which as of 2026-09-04 sits at 6416094 and still tracks
`deploy/targets.json` and three per-row artifacts (§What is withheld). Merging this branch
fixes the tip; only a history rewrite or making the repository private fixes the past.
...
- Dictionary build lineage: `6fcd02755bf3` → `d7a70c5014c5` → `c272da5de196` →
  `3dc8415eccfe` (`tests/test_dictionary.py`). Claims in `benchmark/`, `env/`, `agent/` and
...
machine (step 0 of the smoke test says how): `deploy/model/` (134.2 MB directory, of
which `model.safetensors` is 133.5 MB and over GitHub's file limit) and
`deploy/targets.json` (the question wording).
...
| withheld | reason |
|---|---|
| `module_{1,2,3}_codebook_full.csv`, `raw/`, `dictionary.json` | The COMPASS survey instrument: the three module codebooks (2,804 dictionary entries after the build, `checks.py`; the orphan branch's README counted 3,222 raw codebook lines, not re-verifiable here), including direct-identifier items (name, phone, email, address). Not cleared for public release. |
...
| `out/fusion_task1_overlap.json`, `out/qx_task2_paired.json`, `out/qx_task3_abstention.json` | Per-row retrieval artifacts that quote gold stems; withdrawn from git on 2026-09-03 on this branch. Their figures are in `FUSION.md`, `QUERY_EXPANSION.md` and the tracked smoke report; checksums in `PROVENANCE.md`. **`origin/main` still tracks them at its tip (6416094) until this branch is merged; after that they remain in history from 73f796b / 8f1d9fb (these three) and e446cf8 (`deploy/targets.json`) until the operator rewrites it or makes the repository private.** |
...
**Residual, measured on this tree** by `python src/wording_scan.py` on 2026-09-04 (eight-word
runs from any `dictionary.json` string found in a tracked text file, counted distinct per
file and summed over files): **169 runs across 20 of 129 files**.
...
figures are on that build's pre-fix 1,241-target subset and say so; claims marked
"VERIFIED on build 6fcd02755bf3" are on that earlier build. `PROVENANCE.md` carries the
artifact, checksum, command and commit for each retrieval figure, and the latency table that
replaces the batched 2.94 ms/query figure. Commit shas quoted inside the pipeline documents
(`AGENTS.md`, `DESIGN.md`, `TASKS.md`, `CHANGELOG.md`, `CLAUDE.md`, `.claude/`) belong to
the private pre-publication history and do not resolve here. The public history has two
roots: d0ff436 (the retrieval tree, `main`) and the orphan b3d818d (the pipeline mirror, a
single commit plus one follow-up); the private pipeline history is reachable from neither.
